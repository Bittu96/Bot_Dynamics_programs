...
while(firstStep):        
    ...
    py =   yi+fly -x -2 #- 5*i + 5*(1/pi)*sin(i*pi)
    pz =   hi-flz -abs(sin((0.5*pi/0.5)*t + pi/2))
    
    pxr =   0-frx+1 + 0.5*cos(i*pi - pi/4)
    pyr =  yi+fry -x -2 #- 5*i + 5*(1/pi)*sin(i*pi)
    pzr =  hi-frz -abs(sin((0.5*pi/0.5)*t + pi/2))
    ...
    #torqueCalc(theta1,theta2, th[0],th[2], th[1],th[3])
    Tq = torqueCalc(theta1,theta2, av1, av2, aa1, aa2)
    #print float(Tq[0])
    #plt.plot(t,float(Tq[0]),'ro',markersize = 1)
    #torqueCalc(0.1,0, 0, 0, 0, 0)
